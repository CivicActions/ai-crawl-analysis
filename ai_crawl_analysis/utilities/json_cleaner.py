...
def extract_json_content(text: str) -> str:
    ...
    start_pos = min((text.find(char) for char in ['[', '{'] if text.find(char) != -1), default=-1)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Clean and validate JSON files with mixed content")
    parser.add_argument("file", help="Path to the JSON file to clean")
    parser.add_argument("--expected-keys", type=int, default=6,
    ...
    try:
        ...
        print(f"✅ JSON is valid and complete: {'YES' if is_valid else 'NO'}")
