...
def crawl_analysis(input_csv: str, output_json: str, columns: list):
    ...
    # The hardcoded path is used to ensure the file is saved in a specific location.
    # ! BRITTLE CODE ALERT -  Don't change this path because it is used in the next step. Or update in both places.
    ...
    json_path = Path(__file__).resolve().parent.parent / 'data/crawl-analysis/migration_groups.json'
    ...
    sidebar_response = call_ai(
        ...
        file='data/crawl-analysis/migration_groups.json',
        response_schema=sidebar_schema
    )

    # Write the response to a new JSON file
    sidebar_path = Path('data/crawl-analysis/sidebar.json')
